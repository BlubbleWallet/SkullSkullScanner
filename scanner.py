...
async def scan_token(ca: str):
    async with aiohttp.ClientSession() as session:
        ...
        holders_task = fetch(session, f"https://api.solscan.io/v2/token/holders?token={ca}&limit=10", 
                            headers={"accept": "application/json"})
        
        dex_data, rug_data, holders_data = await asyncio.gather(dex_task, rugcheck_task, holders_task)
        ...
        if dex_data and dex_data.get("pairs"):
            ...
            pair_address = pair.get("pairAddress", ca)
            ...
            dex_paid = pair.get("boosts") is not None
        ...
        if rug_data:
            ...
            if markets:
                lp_burnt_pct = markets[0].get("lp", {}).get("lpBurnedPct", 0) or 0
                lp_burnt_pct = round(lp_burnt_pct * 100, 1) if lp_burnt_pct <= 1 else round(lp_burnt_pct, 1)
            ...
            if top_holders_raw:
                for i, h in enumerate(top_holders_raw[:10]):
                    addr = h.get("address", "???")
                    pct = h.get("pct", 0) * 100 if h.get("pct", 0) <= 1 else h.get("pct", 0)
            ...
            for t in creator_tokens:
                if t.get("mint") == ca:
                    dev_hold_pct = t.get("pct", 0) * 100 if t.get("pct", 0) <= 1 else t.get("pct", 0)
        ...
        if is_honeypot:
            score -= 4
            red_flags.append("🔴 HONEYPOT - Token tidak bisa dijual!")
        if mint_auth:
            score -= 2
            red_flags.append("🔴 Mint ON → Dev bisa cetak token baru")
        if freeze_auth:
            score -= 2
            red_flags.append("🔴 Freeze ON → Dev bisa freeze wallet")
        if lp_burnt_pct < 50:
            score -= 2
            red_flags.append(f"🔴 LP Burnt {lp_burnt_pct}% → Bisa rug pull")
        if top_10_pct > 50:
            score -= 1
            red_flags.append(f"🔴 Top 10 pegang {top_10_pct:.1f}% supply")
        elif top_holders_raw and (top_holders_raw[0].get("pct", 0) * 100 if top_holders_raw[0].get("pct", 0) <= 1 else top_holders_raw[0].get("pct", 0)) > 15:
            score -= 1
            warnings.append(f"⚠️ Top holder > 15% — WHALE ALERT")
        if bundles > 0:
            score -= 1
            red_flags.append(f"🔴 Bundle terdeteksi ({bundles})")
        if snipers > 0:
            score -= 1
            warnings.append(f"⚠️ Sniper terdeteksi ({snipers})")
        ...
        honeypot_str = "🔴 HONEYPOT!" if is_honeypot else "🟢 SAFE (Bisa Dijual)"

        # Build top holders text
        holders_text = "\n".join(top_holders) if top_holders else "Data tidak tersedia"

        # Build red flags text
        flags_text = ""
        if red_flags or warnings:
            all_flags = red_flags + warnings
            flags_text = f"\n\n━━━━━━━━━━━━━━━━━━━━\n🚨 *RED FLAGS ({len(red_flags)} ditemukan)*\n━━━━━━━━━━━━━━━━━━━━\n"
            flags_text += "\n".join(all_flags)
        else:
            flags_text = "\n\n✅ *Tidak ada red flag terdeteksi!*"

        # Short CA
        short_ca = f"{ca[:8]}...{ca[-6:]}"
